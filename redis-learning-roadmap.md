...
> 再补一句（PG DBA 最容易卡住的地方）：**别把 Redis 当"数据库"，把它当"你直接操作内存数据结构的工具箱"。**

### 为什么 Redis 每种类型一套命令，而不是一套 SQL

PG 用一套 SQL 引擎（解析 → 优化器 → 执行计划）查所有表；Redis 把引擎拆掉了，
**每条命令就是某种数据结构的原生操作**，中间没有解析、没有优化器、没有计划漂移：

| 心智 | 说明 |
| --- | --- |
| 没有查询引擎 | 命令 = 结构自带操作：Hash 有 `HGETALL`、ZSet 有 `ZREVRANGE`、Set 有 `SISMEMBER`... |
| 像集合类 API | `HashMap→HSET/HGET`；`TreeMap→ZRANGE`；`HashSet→SADD/SISMEMBER`；`Deque→LPUSH/LPOP` |
| 复杂度在设计时 | PG 运行时靠优化器选路；Redis 靠"选对类型 + 设计好 Key"，选对了就是一条命令 |
| 可预测 | 无优化器 = 命令耗时基本固定（O(1)/O(log N)），无执行计划漂移 |
| 选型口诀 | 主键查一行→Hash；分数/排名范围→ZSet；存在性/集合运算→Set；FIFO→List；单值/计数→String |
