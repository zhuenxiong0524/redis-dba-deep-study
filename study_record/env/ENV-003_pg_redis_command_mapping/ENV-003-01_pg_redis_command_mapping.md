...
- 冒号只是普通字符，Redis 不解析，**层数不限**，作用 = 分层命名空间（类似文件路径 `/a/b/c`）；
- `SCAN 0 MATCH 'dat001:*'` 取第 1 层全部 → `'dat001:str:*'` 收窄到第 2 层 → `'dat001:str:int*'` 精确到对象；
- 生产常见：`order:20260825:1001`（业务:日期分片:订单号）、`user:1:profile`（业务:ID:子对象）；
- 注意：冒号**不影响集群分片路由**（Cluster 看 `{...}` hash tag，不是冒号，见 CLU-001）。
